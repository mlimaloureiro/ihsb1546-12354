...
(function ($) {
	$.jstree.plugin("checkbox", {
		...
		_fn : {
			...
			getAllChecked: function(obj){
				
				obj = !obj || obj === -1 ? this.get_container() : this._get_node(obj);
				
				var elements = obj.find("> ul > .jstree-checked, .jstree-unchecked > ul > .jstree-checked")
				var res = []
				
				
				for (var i=0;i<elements.length;i++){
					res.push(elements[i]);
					res.push(this.getAllChecked($(elements[i])));
				}
			...
			_repair_state : function (obj) {
				...
				else if (obj.hasClass('jstree-checked')) { this.check_node(obj);}
